...
def do_magic(BEGIN_COORDS,END_COORDS,ASP):
    if ASP.type=='P':
       return do_magic_polygon(BEGIN_COORDS,END_COORDS,ASP)
    if ASP.type=='C':
        return do_magic_circle(BEGIN_COORDS,END_COORDS,ASP)
    #AVG_LAT=(BEGIN_COORDS[0]+END_COORDS[0])/2
    #CONV_FACT=6.3781*(10**6)*math.cos(AVG_LAT*math.pi/180)
def do_magic_polygon(BEGIN_COORDS,END_COORDS,ASP):
...
def do_magic_circle(BEGIN_COORDS,END_COORDS,ASP):
    BEGIN=Node(BEGIN_COORDS[0],BEGIN_COORDS[1],start=True)
    ENDING=Node(END_COORDS[0],END_COORDS[1],fin=True)
    ok1=False
    for p in [BEGIN,ENDING]:
        if Geometrics.distance([p.LAT,p.LONG],ASP.centroid)<5/60:
                    ok1=True
                    break
